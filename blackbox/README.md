...
Note: If you need a newer Clash version, simply change the version bounds in `blackbox.cabal` and follow the hints given by Stack.

## src/
This is where the source code of the project lives, as specified in `blackbox.cabal`. It contains a single file, `Example/Project.hs` which starts with:
...
`blackbox.cabal` enabled `NoImplicitPrelude` which enables the use of `Clash.Prelude` here. Next, a function `plus` is defined. It simply adds two numbers. Note that the example (`>>> plus 3 5`) gets executed by the _doctests_ defined for this project and checked for consistency with the result in the documentation (`8`).
...
# Change the license
By default `blackbox.cabal` sets its `license` field to `BSD-2-Clause`. You might want to change this.
